...
double Point::Distance(const Point &segmentStart,
  ...
  const double t =
      ((*this - segmentStart).DotProduct(segmentEnd - segmentStart)) / l2;
  if (t < 0.0) {
...
bool HullFinder::IsPointInsidePolygon(Point v, const vector<Point> &polygon) {
  bool result = false;
  int j = polygon.count() - 1;
  for (int i = 0; i < polygon.count(); i++) {
    if ((polygon[i].y < v.y && polygon[j].y > v.y) ||
        (polygon[j].y < v.y && polygon[i].y > v.y)) {
      if (polygon[i].x + (v.y - polygon[i].y) / (polygon[j].y - polygon[i].y) *
...
bool HullFinder::CheckEdgeIntersection(const Point &p0, const Point &p1,
...
bool HullFinder::CheckEdgeIntersection(const vector<Point> &hull,
                                       Point curEdgeStart, Point curEdgeEnd,
                                       Point checkEdgeStart,
                                       Point checkEdgeEnd) {
...
Point HullFinder::NearestInnerPoint(Point edgeStart, Point edgeEnd,
...
vector<Point> HullFinder::FindConvexHull(const vector<Point> &points) {
...
vector<Point> HullFinder::FindConcaveHull(const vector<Point> &points,
                                          double N) {
